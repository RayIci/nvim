...
local function commit_model(tool)
  local models = vim.g.sidekick_commit_models
  if type(models) == "table" and type(models[tool]) == "string" and models[tool] ~= "" then
    return models[tool]
  end
  return tool == "copilot" and "auto" or nil
end
...
local function with_sidekick_tool(cb)
  local state = require("sidekick.cli.state").get({ attached = true, cwd = true })[1]
    or require("sidekick.cli.state").get({ attached = true })[1]
    or require("sidekick.cli.state").get({ started = true, cwd = true })[1]
    or require("sidekick.cli.state").get({ started = true })[1]
  if state then
    cb(state.tool.name)
    return
  end
...
local function commit_message()
  ...
  with_sidekick_tool(function(tool)
    generate_commit_message(buf, tool)
  end)
end
